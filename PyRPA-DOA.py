...
def Analysis(PicName, location):
    ...
    while local < Key_Value_pair and running == 1:
        ...
        if NowRowKey[local] == '左键':
            ...
            Split = re.split('/', NowRowValue[local])
            mylog('鼠标相对移动', Split)
            Split=(int(Split[0])+int(float(Split[0])*random.uniform(-0.1, 0.1)),int(Split[1])+int(float(Split[1])*random.uniform(-0.1, 0.1)))
            pyautogui.moveRel(xOffset=int(Split[0]), yOffset=int(Split[1]), tween=pyautogui.linear)
...
def bezier_curve(p0, p1, p2, p3, t, noise_std_dev=1):
    """计算增加了噪音的抛物线上t位置的点坐标"""
    # 计算原始的贝塞尔曲线点
    ...
    # 添加高斯噪音
    noisy_x = x + random.gauss(0, noise_std_dev)
    noisy_y = y + random.gauss(0, noise_std_dev)

    return (noisy_x, noisy_y)


def generate_bezier_path(start, control1, control2, end, num_points=80+int(random.uniform(-25, 38))):
    #print(num_points)
    """生成三次贝塞尔曲线上的点路径"""
    path = []
    for i in range(num_points + 1):
        t = i / num_points
        path.append(bezier_curve(start, control1, control2, end, t))
    return path

def move_mouse_bezier_path(start, end, duration, noise_std_dev=1):
    """根据给定的开始点、结束点和移动时间，移动鼠标并添加噪音"""
    # 随机选择控制点以创建非线性路径
    control1 = (start[0] + np.random.randint(50, 150), start[1] + np.random.randint(-100, 100))
    control2 = (end[0] - np.random.randint(50, 150), end[1] - np.random.randint(-100, 100))
    
    # 生成贝塞尔曲线路径
    path = generate_bezier_path(start, control1, control2, end)
    num_points = len(path)
    ...
    for point in path:
        # 在x和y方向添加高斯噪音
        noisy_x = point[0] + np.random.normal(0, noise_std_dev)
        noisy_y = point[1] + np.random.normal(0, noise_std_dev)
        mouse.position = (int(noisy_x), int(noisy_y))
        time.sleep(interval)
        
def randomnumber (ImgPath,end_position):
    # 获取当前鼠标的位置
    start_position = pyautogui.position()
    # 起点位置 (A)
    ImgPaths=ImgPath
    random_number = random.uniform(0.1, 0.5)
    random_number_x=int(random.uniform(-8, 12))
    random_number_y=int(random.uniform(-10, 11))
    random_xy=(random_number_x,random_number_y)
    result_xy = tuple(x + y for x, y in zip(end_position, random_xy))
    
    duration = 0.3+random_number
    #print(result_xy,duration)
    # 移动时间，单位秒
    move_mouse_bezier_path(start_position, result_xy, duration)
    Analysis(ImgPaths,result_xy)
